...
import Comodal from '../modals/Comodal';
...
const Courses = ({ isNavbar }) => {
    ...
    const Course = ({ course }) => {
        ...
        return (
            <div className={`flex flex-col border ${!isNavbar && 'h-[29rem] w-[30%]'} rounded-2xl cursor-pointer hover:scale-105 duration-700 ${isNavbar ? 'pt-3' : 'pt-0'}`}>
                {!isNavbar && <img src={course.image} alt="" className='rounded-t-2x' />}
                ...
                <div className="flex flex-col justify-end h-full">
                    ...
                    {!isNavbar && <div className="flex flex-row w-full border-t border-0 text-lg font-medium">
                        <button
                            ...
                            onClick={() => openBrochure(course.brochure)}
                        >
                            {download}Brochure
                        </button>
                        <button className='flex flex-row w-1/2 rounded-r-2xl rounded-t-[0px_1rem] items-center justify-center bg-primary py-2' onClick={() => openModal(course)}>View Program</button>
                    </div>}
                </div>
            </div>
        );
    ...
    return (
        <div className="flex flex-col">
            ...
            {isModalOpen && <Comodal onClose={closeModal} course={selectedCourse} />}
        </div>
    );
};
